...
moon.on('message', async message => {
    ...
    const filter2 = (reaction, user) => [':arrow_left:', ':arrow_right:'].includes(reaction.emoji.name) && (message.author.id == user.id)
    const Embed = await message.channel.send(` ${current+1}/${pages.length} 페이지`, pages[current])
    await Embed.react(':arrow_left:')
    await Embed.react(':arrow_right:')
